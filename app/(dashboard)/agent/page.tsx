...
function AgentInner() {
  ...
  const [psiSession, setPsiSession] = useState<{
    active: boolean;
  ...
  // PSI Optimization Handlers
  const startPSISession = useCallback(async (riskTier: 'low' | 'medium' | 'high' = 'low') => {
    if (!selectedSiteId) {
      sendWithSite("Please select a site first to optimize PSI.", selectedSiteId, false);
      return;
    }
    try {
      ...
      sendWithSite(
        `PSI Optimization started for ${siteName}.\n\n` +
        `Current Baseline:\n` +
        `• Mobile PSI: ${data.psi_mobile_before}/100\n` +
        `• Desktop PSI: ${data.psi_desktop_before}/100\n` +
        `• Risk Tier: ${riskTier}\n\n` +
        `Deploying first fix...`,
        selectedSiteId,
        false
  ...
  const deployNextPSIFix = useCallback(async (sessionId: string, riskTier: 'low' | 'medium' | 'high') => {
    ...
    try {
      ...
      const data = res.data || res;
      const improvement = data.deployment?.improvement_mobile || 0;

      sendWithSite(
        `Fix deployed: ${data.deployment.fix_id}\n\n` +
        `Improvements:\n` +
        `• Mobile: +${improvement} pts\n` +
        `• Health: ${data.health_check?.healthy ? '✓ Pass' : '✗ Failed'}\n\n` +
        `Review the changes on your live site. Type "approve" to keep this fix and continue, "reject" to rollback, or "stop" to end optimization.`,
        selectedSiteId,
        false
      );
  ...
  const handlePSIUserResponse = useCallback(async (response: string) => {
    if (!psiSession.active || !psiSession.sessionId || !selectedSiteId) return;
    ...
    if (lower === 'approve' || lower.includes('approve')) {
      try {
        ...
        setPsiSession(p => ({ ...p, fixesApplied: p.fixesApplied + 1 }));
        sendWithSite(`✓ Fix approved and applied.\n\nDeploying next fix...`, selectedSiteId, false);
        setTimeout(() => deployNextPSIFix(psiSession.sessionId!, psiSession.riskTier), 1000);
      } catch (err: any) {
        sendWithSite(`Error approving fix: ${err.message}`, selectedSiteId, false);
      }
      ...
      try {
        ...
        sendWithSite(`✓ Fix rolled back. Site restored to previous state.\n\nDeploying alternative fix...`, selectedSiteId, false);
        setTimeout(() => deployNextPSIFix(psiSession.sessionId!, psiSession.riskTier), 1000);
      } catch (err: any) {
        sendWithSite(`Error rolling back: ${err.message}`, selectedSiteId, false);
      }
      ...
      try {
        ...
        sendWithSite(
          ...
          `Summary:\n` +
          `• Fixes Applied: ${results.fixes_deployed || psiSession.fixesApplied}\n` +
          `• Total Improvement: +${results.psi_improvement || psiSession.totalImprovement} pts\n` +
          `• Final Mobile PSI: ${results.psi_mobile_after || '—'}/100\n` +
          `• Duration: ${Math.round((new Date().getTime() - Date.now()) / 60000)} minutes\n\n` +
          `Great work! Your site is now optimized. Monitor it over the next 24 hours to ensure stability.`,
          selectedSiteId,
          false
        );
        setPsiSession({ active: false, sessionId: null, currentIteration: 0, riskTier: 'low', fixesApplied: 0, totalImprovement: 0 });
      } catch (err: any) {
        sendWithSite(`Error getting results: ${err.message}`, selectedSiteId, false);
      }
    }
  }, [psiSession, selectedSiteId, sendWithSite]);
  ...
  const send = useCallback(async (text: string) => {
    ...
    // Check if this is a PSI optimization request
    const lowerText = trimmed.toLowerCase();
    if (lowerText.includes('optimize psi') || lowerText.includes('psi optim')) {
      // Extract risk tier if specified
      const riskTier = lowerText.includes('medium') ? 'medium' : lowerText.includes('high') ? 'high' : 'low';
      setMessages(prev => [...prev, { role: "user", content: trimmed, created_at: new Date().toISOString() }]);
      startPSISession(riskTier);
      return;
    }

    // Check if PSI session is active and this is a response
    if (psiSession.active) {
      setMessages(prev => [...prev, { role: "user", content: trimmed, created_at: new Date().toISOString() }]);
      handlePSIUserResponse(trimmed);
      return;
    }

    // Normal Agent message
    await sendWithSite(trimmed, selectedSiteId, true);
  }, [loading, selectedSiteId, sendWithSite, psiSession.active, startPSISession, handlePSIUserResponse]);
